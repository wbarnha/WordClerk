# WordClerk Privacy Policy

**Last updated:** July 2026

This Privacy Policy describes what data WordClerk, a Microsoft Word add-in, collects, uses, and
shares. It's separate from — and should be read alongside — [WordClerk's Terms of Use](TERMS.md).

> **Short version:** WordClerk has no backend, no user accounts, and collects no personal
...
This policy covers WordClerk itself — the add-in code that runs inside Microsoft Word, published
and maintained by William Barnhart at
[github.com/wbarnha/WordClerk](https://github.com/wbarnha/WordClerk). WordClerk is open source
...
WordClerk is a task-pane add-in: static HTML/JavaScript/CSS that Word loads and runs inside its
own built-in browser control. It is **not** a hosted web application. There is no WordClerk
server, no database, no user account system, and no way for WordClerk's publisher to see what
documents you open, what you type, or how you use the add-in.

## Data WordClerk itself collects

**None.** WordClerk collects no personal information, no usage analytics, no crash reports, and no
...
## What WordClerk touches on your computer

WordClerk only reads and edits the Word document you currently have open, and only when you click
...
dialog for the "Case Law" (file-based) workflow. The Word permission WordClerk requests is limited
...
By default, WordClerk makes **zero network calls**. Several of its workflows (Case Law
...
- **Online Lookup** (part of Manage Hyperlinks) and **Find Hallucinations**: when you select a
  provider and run a scan, WordClerk sends the **text of individual matched citations** (e.g.
  ...
  operated by the nonprofit Free Law Project. You can instead configure WordClerk to send these
...
- If you supply an API token or other credentials for one of these providers, that credential is
  sent (over HTTPS only — WordClerk refuses plain HTTP) to the provider you're authenticating
...
network call WordClerk ever makes lives in that one folder.
...
- Credentials are held **in memory only**, for the current task-pane session. They are never
  written to `localStorage`, `sessionStorage`, cookies, or any WordClerk-controlled server, and are
...
WordClerk retains nothing, because it stores nothing — there is no WordClerk-operated server or
...
policy and retention practices, not WordClerk's.

## Children's privacy

WordClerk is a professional legal-research tool intended for use by attorneys, paralegals, and
...
[github.com/wbarnha/WordClerk/releases](https://github.com/wbarnha/WordClerk/releases).

## Contact

Questions about this policy or WordClerk's data handling can be raised as a
[GitHub issue](https://github.com/wbarnha/WordClerk/issues) — there is no separate support line or
vendor contact, since WordClerk is maintained in the open as an individual open-source project.
