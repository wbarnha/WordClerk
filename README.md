...
| `adm-zip` (via `office-addin-manifest`/`-debugging`/`-dev-settings`/`-project` → `@microsoft/teamsfx-core` → `@microsoft/kiota`) | [GHSA-xcpc-8h2w-3j85](https://github.com/advisories/GHSA-xcpc-8h2w-3j85) — a crafted ZIP triggers a ~4 GB memory allocation (DoS) | High | `<0.6.0` (installed `0.5.12`/`0.5.16`/`0.5.18`) | `^0.6.0` (forced via `overrides`) | Reached only through Microsoft's dev/debug Office-Add-in tooling; never in `dist/`. `npm audit fix --force` would "fix" this by **downgrading** `office-addin-manifest` to `1.9.1` (a breaking regression of our manifest tooling), so instead the `overrides` entry forces the patched `adm-zip@0.6.0` tree-wide while leaving `office-addin-manifest` at its current version. Verified `npm test`/`typecheck`/`lint`/`build` and `office-addin-manifest validate` still work under `0.6.0`. |
